...
struct callable_base
    : callable_base_impl,
      aux::conditional_t<aux::is_class<T>::value, T, aux::none_type> {};
template <typename T>
...
struct is_supported
    : aux::is_same<aux::bool_list<aux::always<TDeps>::value...>,
                   aux::bool_list<(
                       aux::is_constructible<TDeps, TDeps &&>::value &&
...
auto boundable_impl(I &&, T &&)
    -> aux::conditional_t<aux::is_same<T, aux::decay_t<T>>::value,
...
auto boundable_impl(concepts::any_of<Ts...> &&, T &&)
    -> get_any_of_error<decltype(boundable_impl(aux::declval<Ts>(),
                                                aux::declval<T>()))...>;
template <class... TDeps>
...
template <class T> using memory_traits_t = typename memory_traits<T>::type;
} // namespace type_traits
namespace concepts {
...
namespace core {
template <class T, class... Ts> struct array_impl {
  using boost_di_inject__ = aux::type_list<Ts...>;
...
auto has_shared_ptr__(T &&)
    -> aux::is_valid_expr<decltype(std::shared_ptr<T>{})>;
#endif
class singleton {
public:
  template <class, class T,
            class = decltype(has_shared_ptr__(aux::declval<T>()))>
  class scope {
  public:
    template <class T_>
    using is_referable =
  ...
  template <class _, class T> class scope<_, T, aux::true_type> {
  public:
    template <class T_>
    using is_referable =
        typename wrappers::shared<singleton, T>::template is_referable<T_>;
...
template <class T> using scope_traits_t = typename scope_traits<T>::type;
} // namespace type_traits
namespace scopes {
class deduce {
public:
  ...
  public:
    template <class T>
    using is_referable = typename type_traits::scope_traits_t<
        T>::template scope<TExpected, TGiven>::template is_referable<T>;
    template <class T, class TName, class TProvider>
    static decltype(
        typename type_traits::scope_traits_t<T>::template scope<TExpected,
                                                                TGiven>{}
...
struct is_expr
    : aux::integral_constant<
          bool, aux::is_callable_with<TGiven, typename TProvider::injector_t,
                                      Ts...>::value &&
...
public:
  ...
  struct scope<TExpected, TGiven,
    ...
    auto create(const TProvider &provider) noexcept {
      using wrapper =
          detail::wrapper_traits_t<decltype((object_)(*provider.injector_))>;
    ...
    auto create(const TProvider &provider) noexcept {
      using wrapper = detail::wrapper_traits_t<decltype((
          object_)(*provider.injector_, detail::arg<T, TExpected, TGiven>{}))>;
      return wrapper{
          (object_)(*provider.injector_, detail::arg<T, TExpected, TGiven>{})};
    }
    TGiven object_;
  ...
  template <class _, class... Ts> class scope<_, aux::type_list<Ts...>> {
    ...
    struct injector : injector__<Ts>... {
      ...
      T create(const named<no_name, T> &,
               const aux::false_type &) const noexcept {
...
public:
  ...
  auto named() noexcept {
    return dependency<TScope, TExpected, TGiven, T, TPriority>{
  ...
  auto to(std::initializer_list<T> &&object) noexcept {
    using type = aux::remove_pointer_t<aux::remove_extent_t<TExpected>>;
    using dependency = dependency<scopes::instance, array<type>,
  ...
  template <class TConcept, class T,
            __BOOST_DI_REQUIRES(externable<T>::value) = 0,
            __BOOST_DI_REQUIRES_MSG(
                concepts::boundable<deduce_traits_t<TExpected, T>,
                                    aux::decay_t<T>, aux::valid<>>) = 0>
  auto to(T &&object) noexcept {
...
struct config {
  template <class T> static auto provider(T *) noexcept {
...
auto configurable_impl(T &&)
    -> aux::is_valid_expr<decltype(T::provider((injector<T> *)0)),
                          decltype(T::policies((injector<T> *)0))>;
...
template <class T> auto is_configurable(const aux::true_type &) {
  return typename get_configurable_error<
      decltype(providable<decltype(T::provider((injector<T> *)0))>()),
...
struct binder {
  ...
  template <class, class TConcept, class TScope, class TExpected, class TGiven,
            class TName>
  static decltype(auto)
  resolve_impl(aux::pair<TConcept, dependency<TScope, TExpected, TGiven, TName,
                                              override>> *dep) noexcept {
    return static_cast<
        dependency<TScope, TExpected, TGiven, TName, override> &>(*dep);
  }
  ...
  static dependency<TScope, TExpected, TGiven, TName, override> resolve_impl__(
      aux::pair<TConcept,
                dependency<TScope, TExpected, TGiven, TName, override>> *);
...
struct is_referable__ {
  static constexpr auto value = dependency__<
      binder::resolve_t<TInjector, T>>::template is_referable<T>::value;
};
...
struct provider<aux::pair<T, aux::pair<TInitialization, TList<TCtor...>>>,
  ...
  template <class, class... TArgs> struct is_creatable {
    static constexpr auto value =
        provider_t::template is_creatable<TInitialization, T, TArgs...>::value;
  };
  template <class TMemory = type_traits::heap>
  auto get(const TMemory &memory = {}) const {
    return get_impl(
        memory,
        static_cast<const injector__<TInjector> *>(injector_)->create_impl(
            aux::type<TCtor>{})...);
  }
...
struct provider<aux::pair<T, aux::pair<TInitialization, TList<TCtor...>>>,
  ...
  auto get(const TMemory &memory = {}) const {
    return TInjector::config::provider(injector_).template get<T>(
        ...
        static_cast<const injector__<TInjector> *>(injector_)
...
struct init {};
struct with_error {};
template <class> struct copyable;
template <class T>
struct copyable_impl
    : aux::conditional<
          aux::is_default_constructible<typename T::scope::template scope<
              typename T::expected, typename T::given>>::value,
          aux::type_list<>, aux::type_list<T>> {};
template <class... TDeps>
struct copyable<aux::type_list<TDeps...>>
    : aux::join<typename copyable_impl<TDeps>::type...> {};
template <class TDeps> using copyable_t = typename copyable<TDeps>::type;
template <class T, class> struct referable { using type = T; };
template <class T, class TDependency> struct referable<T &, TDependency> {
  using type =
      aux::conditional_t<TDependency::template is_referable<T &>::value, T &,
                         T>;
};
template <class T, class TDependency> struct referable<const T &, TDependency> {
  using type =
      aux::conditional_t<TDependency::template is_referable<const T &>::value,
                         const T &, T>;
};
#if defined(__MSVC__)
template <class T, class TDependency> struct referable<T &&, TDependency> {
...
class injector : injector_base, pool<bindings_t<TDeps...>> {
  ...
  template <class> friend struct pool;
        ...
        aux::is_convertible<
            decltype(dependency__<dependency_t>::template try_create<T, TName>(
                try_provider<ctor_t, injector,
                             decltype(TConfig::provider((injector *)0))>{})),
            T>::value &&
...
public:
  ...
  template <
      template <class...> class T,
      __BOOST_DI_REQUIRES(
          is_creatable<binder::resolve_template_t<injector, aux::identity<T<>>>,
...
private:
  ...
  explicit injector(const from_deps &, TArgs... args) noexcept
      : pool_t{copyable_t<deps>{},
               core::pool_t<TArgs...>{static_cast<TArgs &&>(args)...}} {}
  template <class TInjector, class... TArgs>
  explicit injector(const from_injector &, TInjector &&injector,
...
class injector<TConfig, pool<>, TDeps...> : injector_base,
...
template <class T> struct constructible_impl {
...
public:
  ...
  auto get(const type_traits::direct &, const TMemory &,
           TArgs &&...args) const {
    return new T(static_cast<TArgs &&>(args)...);
  }
  template <class T, class TMemory, class... TArgs>
  auto get(const type_traits::uniform &, const TMemory &,
           TArgs &&...args) const {
    return new T{static_cast<TArgs &&>(args)...};
  }
};
